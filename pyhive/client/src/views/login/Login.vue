<template>
	<div id="login">
		<el-form :rules="rules" label-width="100px" class="form" :model="form" ref="form">
			<div class="title">
				<img src="../../assets/images/logo.png" alt="PyHive">
			</div>
			...
			<el-form-item :label="null" class="submit">
				<el-button type="primary" class="btn btn-login" :loading="loading" @click.native.prevent="onLogin">{{ $lang.buttons.login }}</el-button>
			</el-form-item>
		</el-form>
	</div>
</template>
		...
		methods: {
			...
			onLogin() {
				this.$refs.form.validate(valid => {
					if (valid) {
						this.loading = true;
						this.$http.post(this.$store.state.url.user.auth, this.form
						).then(({data: data}) => {
