<template>
	<div class="panel">
		...
		<div class="panel-body" v-loading="loadData" :element-loading-text="$lang.messages.loading">
			<el-row>
				<el-col :span="8">
					<el-form ref="form" :model="form" :rules="rules" label-width="100px">
						<el-form-item :label="$lang.columns.name" prop="name">
							<el-input v-model="form.name" :placeholder="$lang.messages.enter + ' ' + $lang.columns.name" size="small"/>
						</el-form-item>
						<el-form-item :label="$lang.columns.ip" prop="ip">
							<el-input v-model="form.ip" :placeholder="$lang.messages.enter + ' ' + $lang.columns.ip" size="small"/>
						</el-form-item>
						<el-form-item :label="$lang.columns.port" prop="port">
							<el-input v-model="form.port" :placeholder="$lang.messages.enter + ' ' + $lang.columns.port" size="small"/>
						</el-form-item>
						...
						<el-form-item :label="$lang.columns.username" prop="username" v-if="form.auth">
							<el-input v-model="form.username" :placeholder="$lang.messages.enter + ' ' + $lang.columns.username" size="small"/>
						</el-form-item>
						<el-form-item :label="$lang.columns.password" prop="password" v-if="form.auth">
							<el-input v-model="form.password" type="password" :placeholder="$lang.messages.enter + ' ' + $lang.columns.password" size="small"/>
						</el-form-item>
						<el-form-item>
		...
		methods: {
			...
			onSubmitForm() {
				this.$refs.form.validate((valid) => {
					...
					this.$http.post(
							this.formatString(this.$store.state.url.client.update, {id: this.form.id}),
							this.form
					).then(() => {
